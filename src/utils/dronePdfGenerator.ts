...
const getAnalysisLabels = (type: string | undefined, name: string | undefined) => {
  const t = (type || name || "").toLowerCase();
  let left = "CROP MONITORING";
  let right = (name || type || "Analysis").toUpperCase() + " ANALYSIS";

  if (t.includes("stress") || t.includes("flowering") || t.includes("pest") || t.includes("health")) {
...
const getTotalAreaStats = (d: DroneAnalysisData, isPlantStress: boolean) => {
  ...
  const ha = stressLevel?.area_hectares ?? a.total_area_hectares ?? a.total_stress_area_hectares ?? 0;
  const pct = stressLevel?.percentage ?? a.total_area_percent ?? a.total_stress_percent ?? 0;
...
export const generateDroneDataPDF = async (
  ...
  try {
    ...
    const drawPageHeader = (startY: number, rowH: number = 20) => {
      ...
      doc.text(report.provider ?? "STARHAWK", W - M, startY + 7, { align: "right" });
      doc.setTextColor(100, 100, 100);
      doc.setFontSize(8);
      doc.text(headerCropText, W - M, startY + 12, { align: "right" });
      doc.setFont("helvetica", "normal");
      if (report.survey_date) {
        doc.text(`Survey date: ${report.survey_date}`, W - M, startY + 17, { align: "right" });
      }
      return startY + rowH;
    };
    ...
    doc.setTextColor(30, 30, 30);
    doc.text(
      field.area_hectares != null ? `${Number(field.area_hectares).toFixed(2)} ha` : "N/A",
      infoLeft + infoW / 2 + 24,
      y + 8
    );
    doc.setFont("helvetica", "normal");
    ...
    if (levels.length > 0) {
      ...
      levels.forEach((lv, i) => {
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(255, 255, 255);
        doc.text(`${(lv.percentage ?? 0).toFixed(1)}%`, blockX + blockW / 2, blockY + blockH - 1.5, { align: "center" });
      ...
      if (stats.ha > 0 || stats.pct > 0) {
        ...
        doc.setFontSize(9);
        doc.setFont("helvetica", "bold");
        doc.text(`Total ${report.analysis_name || "Affected Area"}:`, W / 2, bannerY + 6, { align: "center" });
        doc.setFontSize(10);
        const dimText = `${stats.ha.toFixed(2)} ha =`;
        doc.text(dimText, W / 2 - 10, bannerY + 15, { align: "right" });
        doc.setFillColor(13, 110, 97);
        doc.rect(W / 2 - 4, bannerY + 10, 32, 8, "F");
        doc.text(`${Math.round(stats.pct)}% field`, W / 2 + 12, bannerY + 15.5, { align: "center" });
        y = bannerY + 28;
      } else {
        y += 10;
      }
    ...
    const hasSC = sc && (sc.plants_counted != null || sc.average_plant_density != null);
    if (hasSC) {
      if (y > H - 60) { doc.addPage(); y = drawPageHeader(0) + 10; }
      doc.setFontSize(11);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(34, 197, 94);
      ...
      doc.text(`${sc.average_plant_density ?? "N/A"} ${sc.plant_density_unit ?? ""}`, M + 50, y + 16);
      doc.text(sc.planned_plants?.toLocaleString() ?? "N/A", M + 50, y + 24);
      doc.text(`${sc.difference_percent ?? "N/A"}% (${sc.difference_plants?.toLocaleString() ?? ""})`, W / 2 + 50, y + 8);
    ...
    const hasRX = rx && (rx.pesticide_type != null || (rx.rates && rx.rates.length > 0));
    if (hasRX) {
      if (y > H - 70) { doc.addPage(); y = drawPageHeader(0) + 10; }
      doc.setFontSize(11);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(34, 197, 94);
      ...
      doc.text(rx.average_pesticide_amount?.toString() || "N/A", col2 + 40, y + 16);
    ...
    const hasZN = zn && (zn.num_zones != null || (zn.zones && zn.zones.length > 0));
    if (hasZN) {
      if (y > H - 40) { doc.addPage(); y = drawPageHeader(0) + 10; }
      doc.setFontSize(11);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(34, 197, 94);
    ...
    if (mapImage) {
      ...
      if (mapImage.data) {
        ...
        } catch (e) { console.warn("Could not fetch map image for PDF:", e); }
      }
      ...
      if (imgDataUrl) {
        ...
        if (legendLvs.length > 0) {
          ...
          legendLvs.forEach((lv: any, i: number) => {
            ...
            doc.setFontSize(7);
            doc.text(lv.level ?? lv.name ?? "", M + i * blockW + blockW / 2, legendY + 11, { align: "center" });
          });
        }
      }
    }
