...
export const LossOverviewTab = ({ claim, fieldName }: LossOverviewTabProps) => {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Progress Saved", description: "Assessment notes and observations updated." });
    },
    onError: (err: any) => {
      toast({
        title: "Error",
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["assessments", "detail", assessmentId] });
      toast({ title: "Assessment Submitted", description: "The claim is now pending insurer approval." });
    },
    onError: (err: any) => {
      toast({
  ...
  const canSubmit = observations.length > 0 && reportText.trim().length > 0 && (assessment?.droneAnalysisPdfs?.length || 0) > 0;
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Key findings from the physical inspection</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              {observations.map((obs, idx) => (
                <div key={idx} className="flex justify-between items-start gap-3 p-3 rounded-md bg-muted/50 border group">
                  <div className="flex items-start gap-3">
                    <div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-primary shrink-0" />
                    <span className="text-sm">{obs}</span>
                  </div>
                  {!isCompleted && (
                    <button onClick={() => removeObservation(idx)} className="opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-destructive">
                      <X className="h-4 w-4" />
                    </button>
                  )}
                </div>
            ...
            <CardContent className="flex-1 space-y-4">
              ...
              {!isCompleted ? (
                <>
                  ...
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button className="w-full bg-green-600 hover:bg-green-700" disabled={!canSubmit || submitMutation.isPending}>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>Submit Loss Assessment?</AlertDialogTitle>
                        <AlertDialogDescription>
                           This will finalize your evaluation for <strong>{fieldName}</strong> and notify the insurer for payout processing. 
                           You won't be able to edit this assessment afterwards.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction onClick={() => submitMutation.mutate()} className="bg-green-600 hover:bg-green-700">
                          Submit Assessment
                        </AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                ...
                <div className="flex flex-col items-center gap-2 p-4 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <p className="text-sm font-bold text-green-900 uppercase tracking-wide">Assessment Finalized</p>
                  {claim.status === "SUBMITTED" && <p className="text-xs text-green-700">Pending Insurer Approval</p>}
                </div>
              )}
            </div>
          </CardContent>
